...
export const enumVanillaShapes = [
    ...
    { shape: "----CuCu", required: 40 },
    { shape: "RuRuRuRu", required: 70 },
    { shape: "RuRu----", required: 70 },
    { shape: "Cu----Cu", required: 170 },
    { shape: "Cu------", required: 270 },
    { shape: "CrCrCrCr", required: 300 },
    { shape: "RbRb----", required: 480 },
    { shape: "CpCpCpCp", required: 600 },
    { shape: "ScScScSc", required: 800 },
    { shape: "CgScScCg", required: 1000 },
    { shape: "CbCbCbRb:CwCwCwCw", required: 1000 },
    { shape: "RpRpRpRp:CwCwCwCw", required: 3800 },
    { shape: "--Cg----:--Cr----", required: 8, throughputOnly: true },
    { shape: "SrSrSrSr:CyCyCyCy", required: 10000 },
    { shape: "SrSrSrSr:CyCyCyCy:SwSwSwSw", required: 6000 },
    ...
    { shape: "CcRcCcRc:RwCwRwCw:Sr--Sw--:CyCyCyCy", required: 25000 },
    { shape: "Rg--Rg--:CwRwCwRw:--Rg--Rg", required: 25000 },
    { shape: "CbCuCbCu:Sr------:--CrSrCr:CwCwCwCw", required: 50000 },
];
{
...
export function randomizedVanillaStepsShapes(randomizer) {
    ...
    // Level 1
    const throughputOnly = randomizer.nextIntRange(0, 100) < throughputRatio;
    levelsDefs.push({
    ...
    // Levels 2 through 10
    const base = [1, 1, 1, 2, 2, 3, 3, 4, 4];
    const step = [0, 1, 2, 0, 1, 0, 1, 0, 1];
    for (let level = 2; level < 11; ++level) {
        ...
        levelsDefs.push({
            shape: calcRandomShape(
                randomizer,
                getGrowingComplexity(base[index], step[index]),
    ...
    // Levels 11 through 26
    for (let level = 11; level < 27; ++level) {
        const index = level - 11;
        const throughputOnly =
            (level === 14 && throughputRatio === -1) || randomizer.nextIntRange(0, 100) < throughputRatio;
    ...
    // Levels 27 through max
    for (let level = 27; level <= connection.levelsToGenerate; ++level) {
        const throughputOnly = throughputRatio === -1 || randomizer.nextIntRange(0, 100) < throughputRatio;

        levelsDefs.push({
            shape: calcRandomShape(
                randomizer,
                getGrowingComplexity(5, level - 11),
                true,
                true,
                true,
                true,
...
export function randomizedStretchedShapes(randomizer) {
    ...
    for (let i = phaseLength; i < phaseLength * 2; ++i) {
        levelsDefs.push({
            shape: calcRandomShape(
                randomizer,
                getGrowingComplexity(1, i - phaseLength),
                phase[baseBuildingNames.cutter] === 1,
                phase[baseBuildingNames.rotator] === 1,
                phase[baseBuildingNames.stacker] === 1,
                phase[baseBuildingNames.painter] === 1,
    ...
    for (let i = phaseLength * 2; i < phaseLength * 3; ++i) {
        levelsDefs.push({
            shape: calcRandomShape(
                randomizer,
                getGrowingComplexity(2, i - phaseLength * 2),
    ...
    for (let i = phaseLength * 3; i < phaseLength * 4; ++i) {
        levelsDefs.push({
            shape: calcRandomShape(
                randomizer,
                getGrowingComplexity(3, i - phaseLength * 3),
    ...
    for (let i = phaseLength * 4; i < phaseLength * 5; ++i) {
        levelsDefs.push({
            shape: calcRandomShape(
                randomizer,
                getGrowingComplexity(4, i - phaseLength * 4),
    ...
    for (let ii = levelsDefs.length + 1; ii <= levelsToGenerate; ++ii) {
        levelsDefs.push({
            shape: calcRandomShape(
                randomizer,
                getGrowingComplexity(5, ii - 1 - phaseLength * 5),
                true,
                true,
                true,
                true,
    ...
    for (let i_phases = 1; i_phases <= 4; ++i_phases) {
        levelsdefs[i_phases] = {
            shape: calcRandomShape(
                ...
                phase[baseBuildingNames.rotator] <= i_phases,
                phase[baseBuildingNames.stacker] <= i_phases,
                phase[baseBuildingNames.painter] <= i_phases,
...
export function randomizedRandomStepsShapes(randomizer) {
    ...
    for (let i_phases = 0; i_phases <= 4; ++i_phases) {
        for (let i = 0; i < phaselength[i_phases]; ++i) {
            levelsdefs.push({
                shape: calcRandomShape(
                    randomizer,
                    getGrowingComplexity(i_phases, i),
                    phase[baseBuildingNames.cutter] <= i_phases,
                    phase[baseBuildingNames.rotator] <= i_phases,
                    phase[baseBuildingNames.stacker] <= i_phases,
                    phase[baseBuildingNames.painter] <= i_phases,
    ...
    const startAllBuildings = levelsdefs.length;
    for (let ii = startAllBuildings + 1; ii <= levelstogenerate; ++ii) {
        levelsdefs.push({
            shape: calcRandomShape(
                randomizer,
                getGrowingComplexity(5, ii - startAllBuildings - 1),
                true,
                true,
                true,
                true,
...
export function randomizedHardcoreDopamineShapes(randomizer, buildingsCount) {
    ...
    const lastLevelGoal = connection.goal === "vanilla" || connection.goal === "mam" ? 1 : 0;
    for (let i = 2; i <= levelstogenerate - lastLevelGoal; ++i) {
        levelsdefs.push({
            shape: calcRandomShape(
                randomizer,
                getGrowingComplexity(buildingsCount, i - 2),
                phase[baseBuildingNames.cutter] <= buildingsCount,
                phase[baseBuildingNames.rotator] <= buildingsCount,
                phase[baseBuildingNames.stacker] <= buildingsCount,
                phase[baseBuildingNames.painter] <= buildingsCount,
    ...
    if (lastLevelGoal) {
        levelsdefs.push({
            shape: calcRandomShape(
                randomizer,
                buildingsCount === 5 ? getGrowingComplexity(5, levelstogenerate - 2) : 5,
                true,
                true,
                true,
                true,
...
function getAmountByLevel(level) {
    ...
    if (connection.levelsToGenerate <= 120) {
        ...
        for (let i = 26; i < connection.levelsToGenerate; ++i) {
            currentIngame.amountByLevelCache[i] = Math.ceil(
                ((25000 + (175000 * (i + 1)) / connection.levelsToGenerate) *
        ...
        const phaseA = Math.floor(connection.levelsToGenerate / 5);
        const phaseB = Math.floor((connection.levelsToGenerate - phaseA) / 2);
        const phaseC = connection.levelsToGenerate - phaseA - phaseB;
        const amountA = phaseA * 30;
        const amountB = 50000;
        const amountC = 200000;
        const stepB = (amountB - amountA) / phaseB;
        const stepC = (amountC - amountB) / phaseC;
        ...
        for (let i = 0; i < phaseB; ++i) {
            currentIngame.amountByLevelCache[i + phaseA] = polishNumber(
                Math.ceil((amountA + stepB * (i + 1)) * reqMultiplier)
            );
        }
        for (let i = 0; i < phaseC; ++i) {
            currentIngame.amountByLevelCache[i + phaseA + phaseB] = polishNumber(
                Math.ceil((amountB + stepC * (i + 1)) * reqMultiplier)
            );
        }
    }
    return currentIngame.amountByLevelCache[level - 1];
...
function getThroughputByLevel(level) {
    ...
    for (let i = 0; i < 26; ++i) {
        currentIngame.throughputByLevelCache[i] = Math.ceil(
            ((2.999 + i * 0.333) * connection.requiredShapesMultiplier) / 10
        );
    }
    // Vanilla throughput if required multiplier == 10, max 200 is important!
    for (let i = 26; i < connection.levelsToGenerate; ++i) {
        currentIngame.throughputByLevelCache[i] = Math.min(
            ((4 + (i - 26) * 0.25) * connection.requiredShapesMultiplier) / 10,
            200
        );
    }
    return currentIngame.throughputByLevelCache[level - 1];
...
function polishNumber(num) {
    ...
    if (num < 10000) {
        if (num2 < 4) {
            return num - (num % Math.pow(10, pow10 - 1));
        } else {
            return num - (num % (5 * Math.pow(10, pow10 - 1)));
        }
    } else {
...
export function linearUpgradeShapes(randomizer) {
    ...
    const processorsshapes = [];
    const paintingshapes = [];
    for (const shapesarray of [beltshapes, minershapes, processorsshapes, paintingshapes]) {
        shapesarray.push(calcRandomShape(randomizer, 1, false, false, false, false, false));
        for (let nextindex = 1; nextindex < 5; ++nextindex) {
            shapesarray.push(
                calcRandomShape(
                    randomizer,
                    nextindex + 2,
        ...
        for (let i = 0; i < 5; ++i) {
            pair.array.push(
                calcRandomShape(
                    randomizer,
                    pair.amount + i,
                    phase[baseBuildingNames.cutter] <= pair.amount,
                    phase[baseBuildingNames.rotator] <= pair.amount,
                    phase[baseBuildingNames.stacker] <= pair.amount,
                    phase[baseBuildingNames.painter] <= pair.amount,
                    phase[baseBuildingNames.mixer] <= pair.amount,
                    pair.array
                )
            );
        }
...
function constructUpgradeShapes(randomizer, beltshapes, minershapes, processorsshapes, paintingshapes) {
    if (!randomizer) {
        ...
        for (const category of [beltshapes, minershapes, processorsshapes, paintingshapes]) {
            ...
            category.push(
                calcRandomShape(randomizer, 30, true, true, true, true, true, [category[5], category[6]])
            );
        }
    }
    const multiplier = connection.requiredShapesMultiplier;
    const upgradedefs = {
        belt: [
            {
                required: [{ shape: beltshapes[0], amount: Math.ceil((30 * multiplier) / 10) }],
            ...
            {
                required: [{ shape: beltshapes[5], amount: Math.ceil((25000 * multiplier) / 10) }],
                excludePrevious: true,
                improvement: 0,
            },
            {
        ...
        miner: [
            {
                required: [{ shape: minershapes[0], amount: Math.ceil((300 * multiplier) / 10) }],
                excludePrevious: true,
                improvement: 0,
            },
            {
            ...
            {
                required: [{ shape: minershapes[5], amount: Math.ceil((25000 * multiplier) / 10) }],
                excludePrevious: true,
                improvement: 0,
            },
            {
        ...
        processors: [
            {
                required: [{ shape: processorsshapes[0], amount: Math.ceil((500 * multiplier) / 10) }],
                excludePrevious: true,
                improvement: 0,
            },
            {
            ...
            {
                required: [{ shape: processorsshapes[5], amount: Math.ceil((25000 * multiplier) / 10) }],
                excludePrevious: true,
                improvement: 0,
            },
            {
        ...
        painting: [
            {
                required: [{ shape: paintingshapes[0], amount: Math.ceil((600 * multiplier) / 10) }],
                excludePrevious: true,
                improvement: 0,
            },
            {
            ...
            {
                required: [{ shape: paintingshapes[5], amount: Math.ceil((25000 * multiplier) / 10) }],
                excludePrevious: true,
                improvement: 0,
            },
            {
    ...
    for (let i = 9; i <= connection.tiersToGenerate; ++i) {
        upgradedefs.belt.push({
            required: [
...
function calcRandomShape(
    ...
    if (complexity < addBoolean(hasCutter, hasRotator, hasStacker, hasPainter, hasMixer)) {
        throw new Error(
            `Complexity ${complexity} too low for important=${[
                hasCutter,
                hasRotator,
                hasStacker,
                hasPainter,
    ...
    do {
        ...
        const complexityProcessing = complexity - addBoolean(hasPainter, hasMixer);
        if (
            hasStacker &&
            (!hasCutter ||
                complexityProcessing < 3 ||
                (complexityProcessing === 3 && hasRotator) ||
                randomizer.choice([true, true, true, false]))
        ) {
            hash = "";
            const available = getBuildingBools(hasCutter, hasRotator, hasStacker, hasPainter, hasMixer);
            const tasked = getBuildingBools(hasCutter, hasRotator, false, hasPainter, hasMixer);
            /** @type {[number]}*/ const floatCutting = [NONE];
            // Randomize layer count while considering that you need to stack layers
            const layercount = Math.min(
                randomizer.choice([2, 2, 2, 3, 3, 4]),
                complexity + 1 - addBoolean(hasCutter, hasRotator, hasPainter, hasMixer)
            );
            // Subtract the stacking from complexity
            complexity -= layercount - 1;
            // Generate layers
            for (let i = layercount - 1; i > 0; --i) {
                const complexityPart = weightedRandomIntRange(
                    randomizer,
                    0,
                    complexity -
                        addBoolean(
                            tasked.cutter,
                            tasked.rotator,
                            tasked.painter || tasked.mixer,
                            tasked.mixer
                        )
                );
                complexity -= complexityPart;
                const layer = calcRandomLayer(
                    randomizer,
                    complexityPart,
                    available,
                    tasked,
                    getEmptyBuildingBools()
                );
                hash = stackLayers(hash, layer, available, tasked, floatCutting);
            }
            hash = stackLayers(
                hash,
                calcRandomLayer(randomizer, complexity, available, tasked, copyBuildingBools(tasked)),
                available,
                tasked,
                floatCutting
            );
...
function stackLayers(bottom, newlayer, available, tasked, floatCutting) {
    ...
    if (connection.isFloatingLayersAllowed && available.rotator) {
        // rotator only ever available if cutter available
        if (
            floatCutting[0] !== HORIZONTAL &&
            !(
                newlayer.substring(0, 4) !== "----" &&
                bottom.substring(bottom.length - 8, bottom.length - 4) === "----"
            ) &&
        ...
        if (
            floatCutting[0] !== VERTICAL &&
            !(
                newlayer.substring(2, 6) !== "----" &&
                bottom.substring(bottom.length - 6, bottom.length - 2) === "----"
            ) &&
            !(
                (newlayer.charAt(6) !== "-" || newlayer.charAt(0) !== "-") &&
                bottom.charAt(bottom.length - 2) === "-" &&
                bottom.charAt(bottom.length - 8) === "-"
            )
        ) {
            floatCutting[0] = HORIZONTAL;
            tasked.rotator = false;
        ...
        if (floatCutting[0] !== NONE) {
            return ShapeDefinition.fromShortKey(bottom)
                .cloneAndStackWith(ShapeDefinition.fromShortKey(newlayer))
                .getHash();
        }
    }
...
function calcRandomLayer(randomizer, complexity, available, tasked, important) {
    ...
    if (
        ...
        addBoolean(
            ...
            important.rotator,
            important.stacker,
            important.painter || important.mixer,
            important.mixer
        )
    ) {
    ...
    const variant = new Array(10).fill(false);
    variant[0] = true;
    if (available.cutter) {
        if (complexity) {
            variant[1] = true;
        }
        if (available.rotator && complexity >= 2) {
            variant[2] = true;
        }
        if (available.stacker && complexity >= 3) {
            variant[3] = true;
        }
    ...
    if (important.stacker) {
        variant[0] = false;
        if (complexity < 7 || !available.rotator) {
            variant[1] = false;
        }
        if (complexity < 8) {
            variant[2] = false;
    ...
    switch (randomizer.choice(variantPool)) {
        case 0: // Full (fixed pattern + random color)
            ...
            complexityPart = Math.max(
                important.mixer ? 2 : important.painter ? 1 : 0,
                randomizer.nextIntRange(0, complexity + 1)
            );
            complexity -= complexityPart;
            ...
            if (important.rotator || (available.rotator && complexity && randomizer.choice([true, false]))) {
                if (randomizer.choice([true, false])) {
                    ...
                    layer = layer.substring(6, 8) + layer.substring(0, 6);
                }
                tasked.rotator = false;
        ...
        case 1: // Vertical half (using calcRandomHalf)
            ...
            if (isWindmillHalf(layer) && !available.rotator) {
                return "----" + layer;
            } else if (randomizer.choice([true, false])) {
                return "----" + layer;
            } else {
                return layer + "----";
            }
        case 2: // Horizontal half (using calcRandomHalf)
            ...
            layer = "--" + layer + "--";
            if (randomizer.choice([true, false])) {
                layer = layer.substring(4, 8) + layer.substring(0, 4);
            }
            return layer;
        case 3: // Vertical half-half (using calcRandomHalf)
            ...
            complexityPart = weightedRandomIntRange(
                randomizer,
                0,
                complexity -
                    addBoolean(
        ...
        case 4: // Horizontal half-half (using calcRandomHalf)
            ...
            complexityPart = weightedRandomIntRange(
                randomizer,
                0,
                complexity - addBoolean(important.painter || important.mixer, important.mixer)
            );
            complexity -= complexityPart;
        ...
        case 5: // Cut-out
            ...
            layer =
                calcRandomSubShape(randomizer, true) +
                calcRandomColor(randomizer, complexity, available, tasked, important);
            switch (randomizer.nextIntRange(0, 4)) {
                ...
                case 3:
                    return layer + layer + layer + "--";
                default:
                    throw new Error("Illegal random value");
            }
        case 6: // 3-1
            ...
            layer =
                calcRandomSubShape(randomizer, true) +
                calcRandomColor(randomizer, complexityPart, available, tasked, getEmptyBuildingBools());
            ...
            switch (randomizer.nextIntRange(0, 4)) {
                case 0:
                    return single + layer + layer + layer;
                case 1:
                    return layer + single + layer + layer;
                case 2:
                    return layer + layer + single + layer;
                case 3:
                    return layer + layer + layer + single;
                default:
                    throw new Error("Illegal random value");
            }
        case 7: // Cornered
        ...
        case 8: // 4 shapes 1 color
            ...
            shapePool = ["C", "R", "S", "W"];
            for (let i = 0; i < 4; ++i) {
                const index = randomizer.nextIntRange(0, shapePool.length);
        ...
        case 9: // Checkered (aka cloned half)
            ...
            do {
                ...
                layer =
                    layer.substring(0, 2) +
                    calcRandomSubShape(randomizer, true) +
                    calcRandomColor(randomizer, complexity, available, resetTasked, resetImportant);
...
function calcRandomHalf(randomizer, complexity, available, tasked, important, windmillAllowed) {
    ...
    const complexityProcessing =
        complexity - addBoolean(important.painter || important.mixer, important.mixer);
    const pool = [0];
    let part = "";
    if (available.rotator && complexityProcessing >= 2) {
        pool.push(1);
    }
    if (available.rotator && available.stacker && complexityProcessing >= 6) {
        pool.push(2);
    }
    if (important.rotator || important.stacker) {
        pool.splice(0, 1);
    }
    if (important.stacker && pool[0] === 1) {
        pool.splice(0, 1);
    }
    switch (randomizer.choice(pool)) {
        case 0:
            part =
                calcRandomSubShape(randomizer, windmillAllowed) +
                calcRandomColor(randomizer, complexity, available, tasked, important);
            part += part;
            break;
        case 1:
...
function calcRandomColor(randomizer, complexity, available, tasked, important, preferredColor = null) {
    ...
    if (available.painter && complexity) {
        pool.push("r", "g", "b");
    }
    if (available.mixer && complexity >= 2) {
        pool.push("y", "p", "c");
    }
    if (available.mixer && complexity >= 3) {
        pool.push("w");
    }
    ...
    if (important.painter || important.mixer || (complexity >= 4 && pool.length > 1)) {
        pool.splice(0, 1);
    }
    if (important.mixer) {
        pool.splice(0, 3);
    ...
    if (preferredColor) {
        ...
        if (includePreferred) {
            for (let i = pool.length - 2; i > 0; --i) {
                pool.push(preferredColor);
            }
        }
    }
    // Get random color
    const ret = randomizer.choice(pool);
    // Check tasked/important if a (mixed) color is chosen
    if (ret !== "u") {
        tasked.painter = false;
        important.painter = false;
    }
    if (!["u", "r", "g", "b"].includes(ret)) {
        tasked.mixer = false;
        important.mixer = false;
    }
    // Return
    return ret;
}
...
function copyBuildingBools(old) {
    const copy = getEmptyBuildingBools();
    for (const building in old) {
        copy[building] = old[building];
    }
    return copy;
}
...
function updateBuildingBoolCompletions(toupdate, template) {
    for (const building in toupdate) {
        toupdate[building] &= template[building];
    }
    return toupdate;
}
